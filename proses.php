...
$exe = mysql_query("select max(a.UAS) as maks, min(a.UAS) as minus, avg(a.UAS) as rata from nilai_akhir a where a.kode_pelajaran='$data'; ");
...
foreach($row as $rec)  
{  
    $json_array['label']='Statistik Nilai';  
    $json_array['max']=$rec['maks'];  
	$json_array['min']=$rec['minus'];
	$json_array['avg']=$rec['rata'];
	$json_array['kkm']='75';
