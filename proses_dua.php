...
$exe = mysql_query("select max(a.UAS) as maksi, min(a.UAS) as minusi, avg(a.UAS) as ratai from nilai_akhir a where a.kode_pelajaran='$data'; ");
...
foreach($row as $rec)  
{  
    $json_array['labeli']='Statistik Nilai';  
    $json_array['maxi']=$rec['maksi'];  
	$json_array['mini']=$rec['minusi'];
	$json_array['avgi']=$rec['ratai'];
	$json_array['kkmi']='75';
